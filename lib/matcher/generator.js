...
const vuetifyRootPath = join(require.resolve('regme-vuetify/es5/components'), '../../..')
...
const directives = Object.keys(require('regme-vuetify/es5/directives'))
  .filter(val => val !== 'default')

const dir = dirname(require.resolve('regme-vuetify/es5/components'))
...
readdirSync(dir).forEach(group => {
  ...
  const component = require(`regme-vuetify/es5/components/${group}`).default
  ...
  decache(`regme-vuetify/es5/components/${group}`)
})
